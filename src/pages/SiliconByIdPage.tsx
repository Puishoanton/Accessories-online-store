...
import { productAPI, CASESTYPEURL, SILICONSURL } from '../services/productsService'
...
const SiliconByIdPage = () => {
  const { openRef, contentRef, isShow, setIsShow } = useOutside(false)
  const nav = useNavigate()
  const dispatch = useAppDispatch()
  const { id } = useParams()
  const { data, isLoading, isError } = productAPI.useGetCaseByIdQuery({
    product: CASESTYPEURL,
    typeOfProduct: SILICONSURL,
    id: id ? +id - 1 : -1,
  })
  React.useEffect(() => {
    window.scrollTo(0, 0)
...
export default SiliconByIdPage
